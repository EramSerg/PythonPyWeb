...
class TrainView(View):
    def get(self, request):
        ...
        self.answer3 = Entry.objects.filter(tags__name__in=['Кино', 'Музыка'])  # Какие статьи содержат тег 'Кино' или 'Музыка' ?
        self.answer4 = (Author.objects.filter(gender='ж')).count()  # Сколько авторов женского пола зарегистрировано в системе?
        self.answer5 = ((Author.objects.filter(status_rule=True)).count()) / ((Author.objects.all()).count())  # Какой процент авторов согласился с правилами при регистрации?
